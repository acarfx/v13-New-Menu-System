...
module.exports = {
    name: "menü",
    aliases: [],
    description: "Komut ile menü oluşturup rol verdirdip alma bot restlense dahi veritabanında otomatik entegre olmaktadır.",
    global: false,
    permissions: "OWNER",
    mysql: false,
    content: false,
                        ...
                        if(i.customId == "kur") {
                            ...
                            if(kurulcak) {
                                ...
                                kurulcak.Roles.forEach(r => {
                                    ...
                                    emoji: { "id": message.guild.emojiGöster(emojiler.Tag).id},
                                value: r})
                                })
                                ...
                                message.channel.send({content: `${kurulcak.Text}`, components: [listMenu]}).then(async (oluşturuldu) => {
                                    ...
                                    collector.on('collect', async (i) => {
                                        const member = await client.guilds.cache.get(sistem.SERVER.ID).members.fetch(i.user.id)
                                        if (!member) return;
...
Yeni oluşturulmakta olan rol seçim menünüze bir isim belirleyin.`)]}).then(async (isimbelirleme) => {
    ...
    collector.on("collect", async (m) => {
        ...
        if(mesaj == "iptal" || mesaj == "ıptal") {
            return isimbelirleme.edit({content: null, embeds: [new MessageEmbed().setDescription(`${message.guild.emojiGöster(emojiler.Iptal)} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
.then(async (açıklamabelirleme) => {
    ...
    collector.on("collect", async (m) => {
        let mesaj = m.content
        if(mesaj == "iptal" || mesaj == "ıptal") {
            return açıklamabelirleme.edit({content: null, embeds: [new MessageEmbed().setDescription(`${message.guild.emojiGöster(emojiler.Iptal)} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
...
Yeni oluşturulmakta olan rol seçim menünüzde listelenecek rolleri belirtin.`).setFooter(`En az 3 tane, en fazla 25 tane rol ekleyebilirsiniz.`)]}).then(async (rolbelirleme) => {
    ...
    collector.on("collect", async (m) => {
        let mesaj = m.content
        if(mesaj == "iptal" || mesaj == "ıptal") {
           return rolbelirleme.edit({content: null, embeds: [new MessageEmbed().setDescription(`${message.guild.emojiGöster(emojiler.Iptal)} Başarıyla rol seçim menü oluşturma aracı iptal edildi.`)]}).then(x => {
                setTimeout(() => {
